...
export default {
  ...
  setup(_, {emit}) {
    ...
    async function update() {
      const {name, email, avatar} = state.form
      const { data } = await axios.put(`/users/${state.form.id}`, {name, email, avatar});
      console.log(data)
      state.form.email = '';
      state.form.avatar = '';
      state.form.name = '';
    ...
    return { state, isModalOpen, submit, newUserModal, isNewUser, edit, update };
  },
};
</script>
